					...
					else if (FD_ISSET(i, &_writeset)) // connection is ready to be written to
					{
						...
						//  env 
							...
							else if(fork_id == 0) // child process
							{
									...
									setenv("CONTENT_TYPE", "application/x-www-form-urlencoded", 1); // for //POST  an GET request // POOOOST
									setenv("CONTENT_LENGTH", std::to_string(14).c_str(), 1); // for post  request //POoooooooooST
								...
								else // parent process
								{
									...
									while ((nbytes = read(fd, cgi_buff, 1024)) > 0)
									{
										...
										std::string real;
										 real = ok.parse_response_cgi(ret);
										// std::cout << ret << std::endl;
